...
import yio.tro.shotakoe.menu.elements.keyboard.*;
...
public class RenderCustomKeyboardElement extends RenderInterfaceElement{
    ...
    private void renderDarkenOnSwipe() {
        if (cbElement.simpleTabsEngineYio.swipeFactor.getProgress() == 0) return;
    ...
    private void renderTextField() {
        if (cbElement.getFactor().getProgress() < 0.01) return;
        CbTextField textField = cbElement.textField;
        GraphicsYio.setBatchAlpha(batch, cbElement.getShadowAlpha());
        MenuRenders.renderShadow.renderShadow(textField.viewPosition, GraphicsYio.shadowCornerRadius);
        GraphicsYio.setBatchAlpha(batch, alpha);
        MenuRenders.renderRoundShape.renderRoundShape(textField.viewPosition, BackgroundYio.white, GraphicsYio.defCornerRadius);
        GraphicsYio.setBatchAlpha(batch, 1);
    ...
    private void renderTextFieldInternals() {
        if (cbElement.getFactor().getProgress() < 0.5) return;
        renderHint();
        CbTextField textField = cbElement.textField;
