...
public abstract class InterfaceElement<T extends InterfaceElement<T>> {
    ...
    public InterfaceElement(MenuControllerYio menuControllerYio) {
        ...
        setAppearParameters(MenuParams.APPEAR_MOVEMENT, MenuParams.ANIM_SPEED);
        setDestroyParameters(MenuParams.DESTROY_MOVEMENT, 3.5 * MenuParams.ANIM_SPEED);

        setParent(screen);
        initActivationFactor();
        setAnimation(MenuParams.DEF_ANIMATION_TYPE); // after setParent()
    }
    ...
    public float getAlpha() {
        ...
        if (getFactor().isInDestroyState()) {
            if (getFactor().getProgress() >= 0.9f) return 1;
            return (1f / 0.9f) * getFactor().getProgress();
        }
        if (getFactor().getProgress() >= 0.8f) return 1;
        return (1f / 0.8f) * getFactor().getProgress();
    }


    public float getShadowAlpha() {
        if (getFactor().isInDestroyState()) {
            if (getFactor().getProgress() < 0.9f) return 0;
            return 10f * (getFactor().getProgress() - 0.9f);
        }
        if (getFactor().getProgress() < 0.5f) return 0;
        return 2f * (getFactor().getProgress() - 0.5f);
    }
    ...
    private void applyParentViewPosition() {
        ...
        if (parent.appearFactor.getProgress() < 1 && parent.doesAnimationAffectSize()) {
            float parentWidthValue = parent.viewPosition.width / parent.position.width;
            float parentHeightValue = parent.viewPosition.height / parent.position.height;
            viewPosition.x *= parentWidthValue;
            viewPosition.y *= parentHeightValue;
            viewPosition.width *= parentWidthValue;
            viewPosition.height *= parentHeightValue;
        }
    ...
    private boolean isCompletelyDestroyed() {
        if (appearFactor.getProgress() == 0) return true;
    ...
    private void animShotakoe() {
        if (appearFactor.isInAppearState()) {
            ...
            hor = (float) (0.5 * (1 + 0.2 * (1 - getCurrentAnimValue())) * position.width);
            ver = (float) (0.5 * (1 + 0.2 * (1 - getCurrentAnimValue())) * position.height);
            cx = position.x + 0.5f * position.width;
            cy = position.y + 0.5f * position.height;
        }
    ...
    private void animFromTouch() {
        ...
        tempPoint.y += getCurrentAnimValue() * (0.5f * GraphicsYio.height - tempPoint.y);
        tempPoint.x += getCurrentAnimValue() * (0.5f * GraphicsYio.width - tempPoint.x);
    ...
    public boolean isViewPositionNotUpdatedYet() {
        return hasParent() && getFactor().getProgress() < 0.001;
    }
    ...
    public T setParent(InterfaceElement parent) {
        ...
        if (parent != null) {
            if (!(parent instanceof ScrollableAreaYio)) {
    ...
    public float getActivationAlpha() {
        if (activationFactor.getProgress() == 1) return 1;
        return 0.2f + 0.8f * activationFactor.getProgress();
    }
    ...
    public T amplifyRubberBand() {
        setAppearParameters(MovementType.soft_big_rubber_band, MenuParams.ANIM_SPEED);
        return getThis();
    }
    ...
    public boolean isAlphaEnabled() {
        return alphaEnabled || activationFactor.getProgress() < 1;
    }
