...
public class CbTextField {
    ...
    private void moveHighlightStuff() {
        ...
        if (highlightFactor.getProgress() == 0) return;
    ...
    public boolean isHighlighted() {
        return highlightFactor.getProgress() > 0;
    }
    ...
    private void moveCursorFactor() {
        cursorFactor.move();
        if (cursorFactor.getProgress() == 1 && !cursorFactor.isInDestroyState() && System.currentTimeMillis() > timeToBlink) {
            cursorFactor.destroy(MovementType.lighty, 1.4);
        }
        if (cursorFactor.getProgress() == 0 && !cursorFactor.isInAppearState()) {
