...
import yio.tro.shotakoe.stuff.DateYio;
...
public class CalendarManager {
    ...
    public ArrayList<MliMonth> months;
    ...
    public boolean isCompleted(int year, int month) {
        CmmItem cmmItem = getItem(year, month);
        if (cmmItem == null) return false;
        return cmmItem.isFullyCompleted();
    }
    ...
    public void onCalendarDayCompleted(SimulationResults simulationResults) {
        int year = simulationResults.year;
        int month = simulationResults.month;
        int day = simulationResults.day;
        CmmItem cmmItem = getItem(year, month);
        if (cmmItem == null) {
        ...
        cmmItem.addDay(day);
        saveProgress();
        Scenes.calendar.onCalendarDayCompleted();
    }
    ...
    private void initMonths() {
        ...
        while (year != 2020 || monthIndex != 12) {
            MliMonth mliMonth = new MliMonth();
            mliMonth.setValues(year, monthIndex);
            months.add(mliMonth);
            monthIndex--;
            if (monthIndex == 0) {
                monthIndex = 12;
